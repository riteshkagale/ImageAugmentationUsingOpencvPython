...
Folder_name="augmented_image_part1"
...
def superpixel_image(image,segments):
    ...
    cv2.imwrite(Folder_name + "/superpixels-" + str(seg) + "-" + img, output)

def invert_image(image,channel):
    # image=cv2.bitwise_not(image)
    image=(channel-image)
    cv2.imwrite(Folder_name + "/invert-"+str(channel)+ "-" + img, image)
...
def add_light(image, gamma=1.0):
    ...
    if gamma>=1:
        cv2.imwrite(Folder_name + "/light-"+str(gamma)+ "-" + img, image)
    else:
        cv2.imwrite(Folder_name + "/dark-" + str(gamma) + "-" + img, image)
...
def add_light_color(image, color, gamma=1.0):
    ...
    image=cv2.LUT(image, table)
    if gamma>=1:
        cv2.imwrite(Folder_name + "/light_color-"+str(gamma)+ "-" + img, image)
    else:
        cv2.imwrite(Folder_name + "/dark_color" + str(gamma) + "-" + img, image)
...
def hue_image(image,saturation):
    ...
    image = cv2.cvtColor(image, cv2.COLOR_HSV2BGR)
    cv2.imwrite(Folder_name + "/hue-" + str(saturation) + "-" + img, image)
