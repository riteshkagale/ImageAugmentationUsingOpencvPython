...
def emboss_image(image):
    ...
    cv2.imwrite(Folder_name + "/Emboss-" + img, image)

def edge_image(image,ksize):
    image = cv2.Sobel(image,cv2.CV_16U,1,0,ksize=ksize)
    cv2.imwrite(Folder_name + "/Edge-"+str(ksize) + "-" + img, image)
...
def addeptive_gaussian_noise(image):
    ...
    cv2.imwrite(Folder_name + "/Addeptive_gaussian_noise-" + img, image)
...
def salt_image(image,p,a):
    ...
    noisy[coords] = 1
    cv2.imwrite(Folder_name + "/Salt-"+str(p)+str(a) + "-" + img, image)
...
def paper_image(image,p,a):
    ...
    coords = [np.random.randint(0, i - 1, int(num_pepper))
              for i in image.shape]
    noisy[coords] = 0
    cv2.imwrite(Folder_name + "/Paper-" + str(p) + str(a) + "-" + img, image)
...
def salt_and_paper_image(image,p,a):
    ...
    coords = [np.random.randint(0, i - 1, int(num_pepper))
              for i in image.shape]
    noisy[coords] = 0
    cv2.imwrite(Folder_name + "/Salt_And_Paper-" + str(p) + str(a) + img, image)
...
def contrast_image(image,contrast):
    ...
    cv2.imwrite(Folder_name + "/Contrast-" + str(contrast) + "-" + img, image)

def edge_detect_canny_image(image,th1,th2):
    image = cv2.Canny(image,th1,th2)
    cv2.imwrite(Folder_name + "/Edge Canny-" + str(th1) + str(th2) + "-" + img, image)

def grayscale_image(image):
    image= cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(Folder_name + "/Grayscale-" + img, image)
